    ...
    try {
      for (Hand hand : leap.getHands()) {
        for (Finger finger : hand.getFingers()) {
          position = finger.getPosition();
          fill(0, 0, 100);
          ellipse((int) map(position.x, -120, 1000, 0, screenWidth), (int) map(position.y, 0, 500, 0, screenHeight), (int) map(position.z, 0, 100, 10, 50), (int) map(position.z, 0, 100, 10, 50));
        }
      }
    } 
    catch(NullPointerException e) {
